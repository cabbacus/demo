            ...
            <select name="barberry_sguide_select">
                ...
                <?php foreach ( $sguide_list as $sguide_post ): ?>
                    <option value="<?php echo esc_attr( $sguide_post->ID ); ?>" <?php selected( $sguide_post_id, $sguide_post->ID ); ?>>
						<?php echo wp_kses( $sguide_post->post_title, barberry_get_allowed_html() ); ?>
					</option>
                <?php endforeach; ?>
...
if( ! function_exists( 'barberry_sguide_dropdown_save' ) ) {
    function barberry_sguide_dropdown_save( $post_id ){
        if ( isset( $_POST['barberry_sguide_select'] ) && $_POST['barberry_sguide_select'] ) {
            ...
            if ( isset( $_POST['barberry_disable_sguide'] ) && $_POST['barberry_disable_sguide'] == 'on' ) {
                ...
                update_post_meta( $post_id, 'barberry_sguide_select', sanitize_text_field( $_POST['barberry_sguide_select'] ) );
            }
    ...
    function barberry_sguide_display_table_template( $sguide_post, $size_tables ){
        ...
        ?>
            <style type="text/css" data-type="vc_shortcodes-custom-css">
                ...
                <?php echo get_post_meta( $sguide_post->ID, 'barberry_shortcodes_custom_css', true ); ?>
			/* */
            </style>
...
if( ! function_exists( 'barberry_sguide_save_category' ) ) {
    function barberry_sguide_save_category( $post_id ) {
        if ( isset( $_POST['barberry_sguide_category'] ) ) {
            $selected_sguide_category = barberry_clean( $_POST['barberry_sguide_category'] );
            ...
            foreach ( $selected_sguide_category as $selected_sguide_cat ) {
	            update_term_meta( $selected_sguide_cat, 'barberry_chosen_sguide', $post_id );
            }   
            foreach( $terms as $term ){
                if ( !in_array( $term->term_id, $selected_sguide_category ) ) {
					if ( $post_id == get_term_meta( $term->term_id, 'barberry_chosen_sguide', true ) ) {
						update_term_meta( $term->term_id, 'barberry_chosen_sguide', '' );
                    }
                }
            }
        }
        ...
        <ul>
            <?php foreach ( $sguide_cat_list as $sguide_cat ): ?>
                ...
                <li>
					...
					<?php echo wp_kses( $sguide_cat->name, barberry_get_allowed_html() ); ?>
                </li>
            <?php endforeach; ?>
        </ul>
        <?php
